...
import src.examen.parc201911.Tabla;
...
public class BD {
	...
	// T2
	public static boolean updateValor( Tabla tabla, int fila, int columna ) {
		String sentSQL = "";
		try {
			// UpdateTablaDato
			sentSQL = "update Dato set " + tabla.getCabecera( columna ) + "='" + tabla.get( fila, columna ) +
					"' where " + tabla.getCabecera( 1 ) + "='" + tabla.get( fila, 1 ) + "';";
			int val = s.executeUpdate( sentSQL );
			if (val!=1) {
				// Si no se modifica (ERROR), devuelve FALSE
				return false;  
			}
			// UpdateTablaTipo
			sentSQL = "update Tipo set" + " fechaUltModif=" + System.currentTimeMillis() +
					" where nombre='" + tabla.getCabecera( 0 ) + "' and valor ='" + tabla.get( fila, 0 ) + "';";
			val = s.executeUpdate( sentSQL );
			if (val!=1) {
				// Si no se modifica (ERROR), devuelve FALSE
				return false; 
			}
