	...
	private static void sacaVentanasPrueba() {
		try {
			...
			vgt.addMenuAccion("T2", (e) -> {
				HashMap<Integer, TreeSet<Puntuacion>> mJugadores = new HashMap<>();
				for ( int i = 1; i <= 5; i++) {
					mJugadores.put( i, new TreeSet<Puntuacion>() );
				}
				for (int f = 0; f < ta.size(); f++) {
					// f --> Filas
					for ( int n = 1; n <= 5; n++ ) {
						// n --> Nivel
						String nick = (String) ta.get( f,  "nick" );
						Integer puntuacion = (Integer) ta.get( f, "p" + n );
						Double tiempo = (Double) ta.get( f, "t" + n );
						if ( puntuacion != null && tiempo != null ) {
							mJugadores.get( n ).add( new Puntuacion( nick, tiempo, puntuacion ) );
						}
					}
				}
				for (int j = 1; j <= 5; j++) {
					// j --> Nivel
					System.out.println( "Clasificación de nivel " + j + ":" );
					for (Puntuacion punt : mJugadores.get( j )) {
